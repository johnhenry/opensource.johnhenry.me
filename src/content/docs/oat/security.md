...
## Side-effecting bootstrap functions require a trusted sender, not just a valid signature

`extractReleaseManifest`, `extractWebrtcBootstrapPayload`, and the `createAnswerArtifact`/`applyAnswerArtifact` pair (in `@johnhenry/oat-bootstrap`) trigger real side effects — an HTTP fetch, applying live WebRTC session data — so they enforce their own gate via `assertVerified()` rather than trusting every caller to check first: `valid`/`signatureValid` alone isn't enough (an unsigned artifact under `requireSignature: false` is still "valid", and anyone can self-sign with a freshly generated key), so `assertVerified()` also requires an explicit `senderTrusted` signal — true only when the signer is on the caller's own trust list (typically `verifyReceivedArtifact`'s `trustedPublicKeysHex`/`requireExplicitTrust`), never merely that *some* key signed the content. This mirrors the exact trust bar `checkSandboxEligibility()` already holds M6's unsafe-HTML path to, above — bootstrap side effects (a WebRTC session hijack, a manifest-driven fetch to a sender-chosen URL) are no less sensitive. Release-manifest URLs are additionally restricted to `https:` by default (`allowedUrlSchemes`) as an SSRF guard, mirroring the same allowlist approach `@johnhenry/oat-ui`'s sanitizer uses.
